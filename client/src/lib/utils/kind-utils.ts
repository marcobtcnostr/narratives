...
const first = (list) => (list ? list[0] : undefined);
export const fromNostrURI = (s) => s.replace(/^[\w+]+:\/?\/?/, "");
export const urlIsMedia = (url) => !url.match(/\.(apk|docx|xlsx|csv|dmg)/) && last(url.split("://"))?.includes("/");
export const isEmbeddableMedia = (url) => isImage(url) || isVideo(url) || isAudio(url);
export const isImage = (url) => url.match(/^.*\.(jpg|jpeg|png|webp|gif|avif|svg)/gi);
export const isVideo = (url) => url.match(/^.*\.(mov|mkv|mp4|avi|m4v|webm)/gi);
export const isAudio = (url) => url.match(/^.*\.(ogg|mp3|wav)/gi);
/**
 * Groups content parts into link collections when they are consecutive media links
 */
export function groupContent(parts) {
    ...
    parts.forEach((part, index) => {
        if (part.type === LINK && (isImage(part.value.url) ||
            ...
            let nextPartsAreNoops = undefined;
...
export const getLinks = (parts) => pluck("value", parts.filter((x) => x.type === LINK && x.isMedia));
