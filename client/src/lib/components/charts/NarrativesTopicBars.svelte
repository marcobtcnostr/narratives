<!-- src\lib\components\charts\NarrativesTopicBars.svelte -->
<script lang="ts">
	...
	$: data = $narrativesStore; // Reactive statement to listen to narrativesStore updates
	...
	function drawChart() {
		...
		// Aggregate duration by macro_topic
		const durationByMacroTopic = d3.rollup(
			data.map((d) => ({ ...d, macro_topic: d.macro_topic || 'Unknown' })), // Replace null macro_topic with 'Unknown',
			(v) => d3.sum(v, (d) => d.duration),
			(d) => d.macro_topic
		);

		const dataArray = Array.from(durationByMacroTopic, ([macro_topic, duration]) => ({
			macro_topic,
			duration
		...
		const colorScale = d3
			.scaleOrdinal()
			.domain(dataArray.map((d) => d.macro_topic)) // Use the unique macro_topics as the domain
		...
		const x = d3
			.scaleBand()
			.range([0, width])
			.domain(dataArray.map((d) => d.macro_topic))
			.padding(0.1);
		svg
			.append('g')
		...
		svg
			...
			.data(dataArray)
			.join('rect')
			.attr('x', (d) => x(d.macro_topic))
			.attr('y', (d) => y(d.duration))
			.attr('width', x.bandwidth())
			.attr('height', (d) => height - y(d.duration))
			.attr('fill', (d) => (d.macro_topic === 'Unknown' ? '#aaa' : colorScale(d.macro_topic))); // Conditional fill color
	}
</script>

<div bind:this={chartContainer} id="topic-bar-chart" style="width: 100%; height: 100%;"></div>
